...
class SVGToRaphael {
	 ...
	 *
	 */
	public $ignored = array('g', 'path', 'polygon', 'rect', 'circle', 'ellipse', 'switch','text');
	...
	public function __construct( $svg = "") {
		...
		//$svg = str_replace("#linear", "linear", $svg);
	...
	public function initRaphael() {
		...
		if(isset($attrs['viewBox'])) {
       	...
       	/*
$w = (strstr($attrs['width'], "%") !== FALSE) ? "'".$attrs['width']."'" : ceil(str_replace('px', '', $attrs['width']));
       	
       	$h = (strstr($attrs['height'], "%") !== FALSE) ? "'".$attrs['height']."'" : ceil(str_replace('px', '', $attrs['height']));
*/
            
       	
       	
        //$this->addToJs(sprintf("var %s = Raphael('%s', %s, %s, %s, %s);\n", $this->svgName, $this->container, $w, $h, floor($x), floor($y)));
        
        // REPLACED FOR 2.0
        $this->addToJs(sprintf("var %s = Raphael('%s', %s, %s);\n", $this->svgName, $this->container, $w, $h));
	 ...
	 *
	 * @params $string String to append
	 * @params $newLine Boolean to add new line character to string
	 */
	...
	public function drawPath() {
		...
		$this->addToJs('var '.$varName.' = '.$this->svgName.'.path("'.$attrs['d'].'")'.$this->generateAttributes(array('d'), $varName).".data('id', '$varName')".';');
	...
	public function drawPolygon() {
       ...
       // echo $this->getAttribute($this->getCurrentElement(), 'points');
        
        $d = sprintf("M %s %s", $this->getAttribute($this->getCurrentElement(), 'points'), 'z');
        
		$this->addToJs('var '.$varName.' = '.$this->svgName.'.path("'.$d.'")'.$this->generateAttributes(array('points'), $varName).".data('id', '$varName')".';');
	...
	public function drawRect() {
		...
		$this->addToJs('var '.$varName.' = '.$this->svgName.".rect($x, $y, $w, $h)".$this->generateAttributes(array('width', 'height'), $varName).".data('id', '$varName')".';');
	...
	public function drawEllipse() {
		...
		$this->addToJs('var '.$varName.' = '.$this->svgName.".ellipse($cx, $cy, $rx, $ry)".$this->generateAttributes(array('cx', 'cy', 'rx', 'ry'), $varName).".data('id', '$varName')".';');
	...
	public function drawCircle() {
		...
		$this->addToJs('var '.$varName.' = '.$this->svgName.".circle($x, $y, $r)".$this->generateAttributes(array('cx', 'cy', 'r',), $varName).".data('id', '$varName')".';');
	...
	public function drawText() {
        ...
        if ( isset($this->getCurrentElement()->tspan) && $text == "" ) {
        	...
        	for ( $i = 0, $len = count($this->getCurrentElement()->tspan) -1; $i <= $len; $i++) {
        		$text .= $this->getCurrentElement()->tspan[$i].'\n';
        	}
        	
        	//$text = $this->getCurrentElement()->tspan;
		...
		$this->addToJs('var '.$varName.' = '.$this->svgName.".text($x, $y, '$text')".$this->generateAttributes(array('y', 'x'), $varName).".data('id', '$varName')".';');
	 ...
	 *
	 * @params $attr Requested attribute
	 * @returns string 
	 */
	 ...
	 *
	 * @params Array $ignored Array holding attributes to ignore
	 * @params String $var Object getting attributes assigned to
	 * @returns string 
	 */
	private function generateAttributes( $ignored = array(), $var = "" ) {
		...
		if (isset($this->getCurrentElement()->attributes()->style)) {
			...
			$vals = explode(';', $this->getCurrentElement()->attributes()->style);
		...
		foreach ($attrs as $key => $value) {
			...
			// CHECK TO MAKE SURE WE CAN USE THIS ATTRIBUTE
				if (!in_array($key, $ignored)) {
					...
					} elseif ( ($k == "fill") && (preg_match('/^url\(#(.+)\)/', $v, $m) >= 1) ) {
				...
				//echo $this->getCurrentElement()->attributes()->transformG." = $type\n";
				
				if ( strstr($this->getCurrentElement()->attributes()->transformG, $type) !== FALSE ) {
	...
	private function processGradient( $id ) {
		...
		if ( array_key_exists('xlink-href', $this->gradients[$id]) ) {
			...
			$gradient = array_merge($this->gradients[$id], $this->gradients[$second]);	
		...
		switch ($gradient['type']) {
			...
			case "linear":
				...
				$angle = $this->angle($gradient['x1'], $gradient['y1'], $gradient['x2'], $gradient['y2']);
				...
				foreach ( $gradient['stops'] as $stop ) {
					...
					$rgba = sprintf("rgba(%s, %s, %s, %s)", $rgb['r'], $rgb['g'], $rgb['b'], $opacity);
			...
			case "radial":
				...
				$g = "r(".(str_replace("%", "", $gradient['fx']) / 100).", ".(str_replace("%", "", $gradient['fy']) / 100).")";
	...
	public function generateJs() {
		echo $this->js;
		//echo $this->angle(85, 122, 432, 432);
	}
	...
	private function __call($method, $arguments) {
		...
		if (strstr($method, 'draw') !== FALSE) {
			//throw new Exception('Unsupported element: '.htmlentities('<'.strtolower(str_replace("draw", "", $method)).' />'));
		}
